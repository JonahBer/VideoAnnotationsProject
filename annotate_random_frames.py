...
class AnnotatorApp:
    ...
    UNSEEN_VIDEO_BONUS = 10.0    # additive weight for videos with zero annotations
    VIDEO_POS_ALPHA = 5.0        # additive weight per unit of positive weight on a video (yes=1, perfect=2)

    def __init__(self, root: tk.Tk, video_dir: str):
        self.root = root
        self.video_dir = video_dir
        self.annotation_file = os.path.join(self.video_dir, "Charlote_Videos/_frame_annotations2.txt")

        # ---- Cache videos
        self.videos: List[str] = list_videos(self.video_dir)
        if not self.videos:
            messagebox.showerror("No videos", f"No video files found in:\n{self.video_dir}")
            root.destroy()
            return
        self.video_name_by_path = {p: os.path.basename(p) for p in self.videos}
        self.video_path_by_name = {os.path.basename(p): p for p in self.videos}

        # ---- Canonical annotations: {video: {ts_str: label}}
        ...
        self.annotation_events: List[Tuple[str, str, str]] = []  # (video, ts_str, label)
    ...
    def _try_show_local_followup(self, video_name: str, center_ts_str: str) -> bool:
        ...
        # Try both sides (+/-) and a few radii; keep it "immediate".
        radii = [
        ...
        for t in candidates:
            ...
            # Must be at least MIN_GAP_SEC from ANY annotation (including the perfect we just added)
            if nearest < self.MIN_GAP_SEC:
                continue
...
def main():
    ...
    VIDEO_DIR = r"C:\Users\bergs\videoProject"
    # VIDEO_DIR = r"D:\NewFolder(3)\Grace\All Content\Videos"
    # -----------------------------------------------------------
